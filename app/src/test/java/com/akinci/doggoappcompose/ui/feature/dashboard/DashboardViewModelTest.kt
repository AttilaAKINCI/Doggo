package com.akinci.doggoappcompose.ui.feature.dashboard
...
class DashboardViewModelTest {
    ...
    fun setup() {
        ...
        val breedData: BreedListResponse = moshi.adapter(BreedListResponse::class.java).fromJson(GetBreedListResponse.breedList)!!
    ...
    fun `select breed`() = runBlockingTest {
        val subBreedData: SubBreedListResponse = moshi.adapter(SubBreedListResponse::class.java).fromJson(GetSubBreedListResponse.subBreedList)!!
        val breed = "hound"
    ...
    fun `validate breed selected, sub breed not selected, empty sub breed list`() = runBlockingTest {
        ...
        val subBreedData = SubBreedListResponse(message = listOf(), status = "success")
        coEvery { doggoRepository.getSubBreedList(breed = breed) } returns flow { emit(NetworkResponse.Success(subBreedData)) }
